...
def render():
    ...
    with col_form:
        with st.form("form_empresa", clear_on_submit=False):
            ...
            col_razao_social, col_nome_fantasia = st.columns(2)
            nome_razao_social = col_razao_social.text_input("Razão Social *", value=cfg_salvo.get("NOME", ""), placeholder="EMPRESA LTDA")
            nome_fantasia = col_nome_fantasia.text_input("Nome Fantasia",  value=cfg_salvo.get("FANTASIA", ""), placeholder="EMPRESA")

            col_cnpj, col_ie = st.columns(2)
            cnpj_raw = col_cnpj.text_input("CNPJ *", value=cfg_salvo.get("CNPJ", ""), placeholder="00.000.000/0000-00",
                                     help="14 dígitos — pode digitar com ou sem máscara")
            ie = col_ie.text_input("Inscrição Estadual *", value=cfg_salvo.get("IE", ""), placeholder="0612345678")

            col_uf, col_municipio = st.columns([1, 2])
            uf = col_uf.selectbox("UF *", UFS,
            ...
            mun_sel = col_municipio.selectbox("Município *", nomes_mun,
            ...
            inscricao_municipal = st.text_input("Inscrição Municipal (IM)", value=cfg_salvo.get("IM", ""),
            ...
            col_logradouro, col_numero, col_cep = st.columns([3, 1, 1])
            logradouro = col_logradouro.text_input("Logradouro", value=cfg_salvo.get("END", ""), placeholder="RUA DAS FLORES")
            numero = col_numero.text_input("Número", value=cfg_salvo.get("NUM", ""), placeholder="100")
            cep = col_cep.text_input("CEP", value=cfg_salvo.get("CEP", ""), placeholder="66000000")
            
            col_complemento, col_bairro = st.columns(2)
            complemento  = col_complemento.text_input("Complemento", value=cfg_salvo.get("COMPL", ""), placeholder="SALA 10")
            bairro = col_bairro.text_input("Bairro", value=cfg_salvo.get("BAIRRO", ""), placeholder="CENTRO")
            
            col_telefone, col_email = st.columns(2)
            telefone  = col_telefone.text_input("Telefone", value=cfg_salvo.get("FONE", ""), placeholder="91912345678")
            email = col_email.text_input("E-mail", value=cfg_salvo.get("EMAIL", ""), placeholder="fiscal@empresa.com.br")

            st.markdown('<div class="section-title" style="margin-top:16px">Configuração fiscal</div>', unsafe_allow_html=True)
            col_perfil_apresentacao, col_indicador_atividade = st.columns(2)
            
            perfil_opt = {"A — Escrituração completa": "A",
                          "B — Escrituração simplificada": "B",
                          "C — Escrituração especial": "C"}
            indicadro_atividade_opt   = {"0 — Industrial ou equiparado": "0",
                          "1 — Outros (comércio / serviços)": "1"}
            
            perfil_lbl = col_perfil_apresentacao.selectbox("Perfil de Apresentação *",
                list(perfil_opt.keys()),
                index=list(perfil_opt.values()).index(cfg_salvo.get("IND_PERFIL", "A")))
            
            ativ_lbl = col_indicador_atividade.selectbox("Indicador de Atividade *",
                list(indicadro_atividade_opt.keys()),
                index=list(indicadro_atividade_opt.values()).index(cfg_salvo.get("IND_ATIV", "1")))

            st.markdown('<div class="section-title" style="margin-top:16px">Contabilista</div>', unsafe_allow_html=True)
            
            col_nome_contador, col_cpf_contador = st.columns(2)
            cont_nome = col_nome_contador.text_input("Nome do Contador", value=cfg_salvo.get("CONT_NOME", ""), placeholder="JOAO DA SILVA")
            cont_cpf = col_cpf_contador.text_input("CPF do Contador",  value=cfg_salvo.get("CONT_CPF", ""),  placeholder="12345678901")
        ...
        if submitted:
            ...
            dados = {
                "NOME": nome_razao_social.strip().upper(),
                "FANTASIA": nome_fantasia.strip().upper(),
                "CNPJ": cnpj_limpo,
                "IE": ie.strip(),
                "UF": uf,
                "COD_MUN": cod_mun.strip(),
                "IM": inscricao_municipal.strip(),
                "SUFRAMA": suframa.strip(),
                "END": logradouro.strip().upper(),
                "NUM": numero.strip(),
                "CEP": re.sub(r"\D", "", cep),
                "COMPL": complemento.strip().upper(),
                "BAIRRO": bairro.strip().upper(),
                "FONE": re.sub(r"\D", "", telefone),
                "EMAIL": email.strip().lower(),
                "IND_PERFIL": perfil_opt[perfil_lbl],
                "IND_ATIV": indicadro_atividade_opt[ativ_lbl],
